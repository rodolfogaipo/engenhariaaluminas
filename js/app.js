...
async function renderView(tab) {
  ...
  if (tab === 'admin' && Auth.isAdmin()) return renderWip(view, 'Painel do Administrador', 'Comparativo de equipe, cadastros, aprovações, avisos e treinamento chegam nos próximos passos.');
  if (tab === 'servicos') return renderWip(view, 'Serviços', 'Lançamento e listagem de serviços (CNP, corte de tecido, corte de espuma, etc.) chega no próximo passo.');
  if (tab === 'corte') return renderWip(view, 'Plano de Corte', 'Acompanhamento de CNPs em corte chega no próximo passo.');
...
async function renderDashboard(view) {
  ...
  view.innerHTML = `
    ...
    <p class="section-sub">Seu desempenho aparece aqui assim que houver serviços lançados.</p>

    <div class="stat-grid">
      <div class="card"><div class="stat"><div class="stat__value">0</div><div class="stat__label">Projetos na semana</div></div></div>
      <div class="card"><div class="stat"><div class="stat__value">—</div><div class="stat__label">Nota</div></div></div>
      <div class="card"><div class="stat"><div class="stat__value">—</div><div class="stat__label">% da Meta</div></div></div>
      <div class="card"><div class="stat"><div class="stat__value">—</div><div class="stat__label">Tendência</div></div></div>
    </div>

    <div class="card" style="margin-top:16px">
      <div class="empty">
        <div class="empty__title">Ainda não há dados para calcular seu desempenho</div>
        <div class="empty__sub">Assim que os módulos de lançamento de serviço estiverem prontos (próximo passo), seus números e gráficos aparecem automaticamente aqui.</div>
      </div>
    </div>
  `;
}
