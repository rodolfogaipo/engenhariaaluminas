    ...
    servicos.forEach((s) => {
      if (s.funcionarioId === funcionarioId && s.dataFinal) {
        eventos.push({
          ...
          // itens trazidos da planilha antiga vêm em lotes (vários
          // cortes baixados no mesmo dia) — não representam o ritmo
          // real de uso do app, então não devem inflar a Meta
          importado: !!s.importadoDaPlanilha,
        });
      }
    });
    ...
    planoCorte.forEach((p) => {
      if (p.funcionarioCorteId === funcionarioId && p.dataFinalCorte) {
        eventos.push({
          ...
          erros: 0,
          errosNovos: 0,
          importado: !!p.importadoDaPlanilha,
        });
      }
    });
